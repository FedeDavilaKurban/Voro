...
class Recon:
    ...
    def __init__(self, \
                 data_ra, data_dec, data_z, bs, \
                 bias=1., f=0., smooth=0.25, nbins=64, \
                 padding=200., opt_box=0, nthreads=1):
        ...
        dat.get_cart(cosmo=cosmo) #Shouldnt do anything
    ...
    #def compute_box(self, padding=200., optimize_box=0):
    ...
    def iterate(self, iloop, save_wisdom=1, verbose=1):
        ...
        fastmodules.allocate_gal_cic(deltag, dat.newx, dat.newy, dat.newz, np.ones(len(dat.newx)), dat.size, self.xmin, self.ymin, self.zmin, self.box, nbins, 1)
        ...
        shift_x, shift_y, shift_z = self.get_shift(dat.newx, dat.newy, dat.newz, psi_x.real, psi_y.real, psi_z.real)
        ...
        for ii in range(2):
            for jj in range(2):
                for kk in range(2):
                    ...
                    pos = (iii, jjj, kkk)
                    #print(pos)
                    #pos[np.where(pos==64)]=0 #Para periodicidad
